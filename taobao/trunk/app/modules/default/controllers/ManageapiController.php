...
class ManageapiController extends Zend_Controller_Action
{
	...
	public function sysgiftAction()
	{
		...
		$uid = $this->_request->getParam('uid');
		$compensation->setUid($uid);
		$compensation->setFeedTitle('恭喜获得淘宝论坛奖励。');
		$total = 1;
...
//				'title' => array('cardName' => '加速卡II'),
		...
		if($gold > 0){
			...
			if($uidlist) {
				foreach($uidlist as $v) {
					$logger->report('801', array($v, $gold, 0));
				}
			}
	...
	public function refreshAction()
	{
		...
		$result = Hapyfish2_Island_Tool_Refresh::all($name);
		$data = array('result' => $result);
		$this->echoResult($data);
	}
	...
	{
		Hapyfish2_Island_Bll_AppInfo::loadToAPC();
		$data = array('result' => 1);
		$this->echoResult($data);
	}
	...
	{		
		...
		$ok = Hapyfish2_Island_Bll_AppInfo::update($appId, $info);
		if ($ok) {
			Hapyfish2_Island_Tool_Refresh::all('appinfo');
		}
		$result = $ok ? 1 : 0;
		$data = array('result' => $result);
		$this->echoResult($data);
