...
class Hapyfish2_Platform_Dal_User
{
    ...
    public function getInfo($uid)
    {
        ...
        $sql = "SELECT uid,puid,name,nick,figureurl,gender,create_time FROM $tbname WHERE uid=:uid";
    ...
    public function add($user)
    {
        ...
        $sql = "INSERT INTO $tbname (uid, puid, name, nick, gender, figureurl, create_time) VALUES"
              . '(' . $uid . ',' . $puid . ',' . $name . ',' . $nick . ',' . $gender . ',' . $figureurl . ',' . $create_time . ')'
    ...
    public function getNewUid($dbId, $tbId, $start_time, $end_time)
    {
    	$tabname = 'platform_user_info_' . $tbId;
    	
    	$sql = "SELECT COUNT(uid) FROM $tabname WHERE create_time BETWEEN $start_time AND $end_time";
