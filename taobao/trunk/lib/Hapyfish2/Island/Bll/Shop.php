...
class Hapyfish2_Island_Bll_Shop
{
    ...
    public static function saleItemArray($uid, $itemArray)
    {
        ...
        if ($result['status'] != 1) {
	        ...
	        try {
	        	$num = count($itemArray);
	        	Hapyfish2_Island_HFC_Achievement::updateUserAchievementByField($uid, 'num_28', $num);
     ...
     * @return array
     */
    public static function saleCard($uid, $cid, $num)
    {
        ...
        if ( !$userCard || !isset($userCard[$cid]) || $userCard[$cid]['count'] < $num) {
        	return $result;
        }

        $userCard[$cid]['count'] -= $num;
		$userCard[$cid]['update'] = 1;
        $ok = Hapyfish2_Island_HFC_Card::updateUserCard($uid, $userCard);
        	
        if ($ok) {
        	$salePrice = $cardInfo['sale_price']*$num;
			Hapyfish2_Island_HFC_User::incUserCoin($uid, $salePrice);
        	
        	$result['status'] = 1;
        	$result['coinChange'] = $salePrice;
        	$result['goldChange'] = 0;
    ...
    public static function buyItemArray($uid, $itemArray)
    {
        ...
        for ($i=0, $iCount = count($itemArray); $i<$iCount; $i++ ) {
            ...
            //type,1x->background,2x->building,3x->plant,4x->card,5x->图纸，6x->材料
            //background
            if ($itemType == 1) {
            ...
            {
            	$mList = Hapyfish2_Island_Cache_Compound::getBasicInfo();
            	if($itemType == 5){
            		$check = Hapyfish2_Island_Bll_Compound::checkCanBuy($uid, $cid);
            		if($check){
            			 $result['content'] = $check;
                    	 return $result;
            		}
            	}
            	$mInfo = $mList[$type][$cid];
                if (!$mInfo || $mInfo['can_buy'] != 1) {
                    $result['content'] = 'serverWord_148';
                    return $result;
                }
                if ($mInfo['price_type'] == 1) {
                    $needCoin += $mInfo['price'] * $itemArray[$i]['num'];
                }
                else if ($mInfo['price_type'] == 2) {
                    $needGold += $mInfo['price'] * $itemArray[$i]['num'];
                }
        ...
        $resultofM = Hapyfish2_Island_Bll_Compound::buyItem($uid, $mArray);
		$costCoin = $resultOfBackground['coin'] + $resultOfBuilding['coin'] + $resultOfPlant['coin'] + $resultOfCard['coin'] + $resultofM['coin'];
		$costGold = $resultOfBackground['gold'] + $resultOfBuilding['gold'] + $resultOfPlant['gold'] + $resultOfCard['gold'] + $resultofM['gold'];
...
//				    if ($now < $chrismasTime) {
//				    	info_log($uid . ',' . $background['name'] . 'x1', 'chrismasBuy'); 
//				    }
//				}
//			}
    ...
    public static function buyBuildingOnIsland($uid, $buyBuildingAry, $isVip = 0, $userLevel = 0, $userCurrentIsland)
    {
        ...
        foreach ($buyBuildingAry as $building) {
        	...
        	if ($ok2) {
				//圣诞节期间统计购买xx
				if (66821 == $building['cid']) {
					$chrismasTime = strtotime('2011-12-27 23:59:59');
				    if ($now < $chrismasTime) {
				    	info_log($uid . ',' . $building['name'] . 'x1', 'chrismasBuy'); 
				    }
				}
        	}
        }
    ...
    public static function buyPlantOnIsland($uid, $buyPlantAry, $isVip = 0, $userLevel = 0, $userCurrentIsland)
    {
    	...
    	foreach ($buyPlantAry as $plant) {
			...
			if ($ok2) {
				//圣诞节期间统计购买
				if (in_array($plant['cid'], array(64932, 128132, 128032, 65531, 128532, 128432))) {
					$chrismasTime = strtotime('2011-12-27 23:59:59');
				    if ($now < $chrismasTime) {
				    	info_log($uid . ',' . $plant['name'] . 'x1', 'chrismasBuy'); 
				    }
				}
			}
    	}
        ...
        foreach ($cardArray as $card) {
			...
			if ($card['price_type'] == 1) {
				...
				if ($ok2) {
					$now = time();
					//圣诞节期间统计购买
					if (in_array($card['cid'], array(127041, 127141))) {
						$chrismasTime = strtotime('2011-12-27 23:59:59');
					    if ($now < $chrismasTime) {
					    	info_log($uid . ',' . $card['name'] . 'x' . $card['count'], 'chrismasBuy'); 
					    }
					}
				}
			}
