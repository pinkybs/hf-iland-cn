...
class Hapyfish2_Island_Bll_Plant
{
	public static function getAllOnIsland($ownerUid, $uid, $islandId, $highcache = false)
    {
		...
		if ($data) {
			...
			foreach ($data['plants'] as $item) {
			    ...
			    if(isset($item['act_name'])) {
		    		$str = explode('_', $item['act_name']);
		    		if($str[1] == 'airfield') {
		    			$plant['canFind'] = 0;
		    			$airCid = $item['cid'];
		    			$airInfo = Hapyfish2_Island_Cache_Compound::getAirRemainVisitor($uid, $airCid);
		    			$remainVisitors = $airInfo['remain_visitor_num'];
		    			$receiveTime = $airInfo['receive_time'];	
		    			$lastTime = $receiveTime + 3600 - $now;
		    			if($lastTime <= 0) {
		    				$plant['downPeopleTime'] = 0;
		    				$plant['currentPeopleNum'] = (int)$remainVisitors;
		    			} else {
		    				$plant['downPeopleTime'] = $lastTime;
		    			}
		    		}
    ...
    public static function handlerPlant(&$item, $now, $home = true, $hasSteal = 0)
    {
    	...
    	if(isset($item['endureRemain'])){
    		$plant['endureRemain'] = $item['endureRemain'] - $now > 0? $item['endureRemain'] - $now : 0;
    	}
