...
class Hapyfish2_Island_Bll_Island
{
    ...
    public static function initIsland($ownerUid, $uid, $checkPraise = false, $islandId)
    {
    	...
    	//补偿礼包
		$valEndTime = strtotime('2012-04-27 23:59:59');	
    	if ($nowTime <= $valEndTime) {
			$valKey = 'ev:valday:gift:0420:' . $uid;
			$valCache = Hapyfish2_Cache_Factory::getMC($uid);
			$valData = $valCache->get($valKey);
	
			if ($valData === false) {
				//发放奖励
				$compensation = new Hapyfish2_Island_Bll_Compensation();
			
				//$compensation->setCoin(200000);
				//$compensation->setGold(10, 0);
				$compensation->setItem(183741, 10);
				$compensation->setItem(183841, 3);
				$ok = $compensation->sendOne($uid, '畅玩赛鱼大礼包:');
				
				if ($ok) {
					info_log($uid, 'ev:valday:gift:0420');
					
					$valCache->set($valKey, 1, $valEndTime);
				}
			}
    	}
    ...
    public static function diyIsland($uid, $changesAry, $removesAry)
    {
        ...
        if (!empty($changePlantList) || !empty($removePlantList)) {
        	...
        	foreach ($changePlantList as $id => $item) {
            	...
            	if($plant) {
            		if(isset($plant['act_name'])){
            			$str = explode('_', $plant['act_name']);
                        if($str[0] == 'miracle'){
                            if(!isset($plant['endureRemain'])){
                            	$plant['endureRemain'] = time()+3600*12;
                            }
                        }
            		}
					if (isset($ids[$id])) {
