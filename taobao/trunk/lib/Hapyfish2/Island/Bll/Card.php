...
class Hapyfish2_Island_Bll_Card
{
	...
	public static function useCard($uid, $ownerUid, $cid, $itemId)
	{
		...
		if ($id == 271) {
			...
			$resultVo = self::blackCard($uid, $ownerUid, $cid);
			if(isset($resultVo['resultVo']['cardStates'])){
                $resultVo['cardStates'] = $resultVo['resultVo']['cardStates'];
                unset($resultVo['resultVo']['cardStates']);
			}
		}
		else if ( $id == 1303 ) {//金币赠送卡
			$resultVo = self::sendCoinCard($uid, $ownerUid, $cid);
			if(isset($resultVo['resultVo']['cardStates'])){
                $resultVo['cardStates'] = $resultVo['resultVo']['cardStates'];
                unset($resultVo['resultVo']['cardStates']);
			}
			...
			$resultVo = self::shareMammonCard($uid, $ownerUid, $cid);
			if(isset($resultVo['resultVo']['cardStates'])){
                $resultVo['cardStates'] = $resultVo['resultVo']['cardStates'];
                unset($resultVo['resultVo']['cardStates']);
			}
	 ...
	 * @return array $resultVo
	 */
	public static function blackCard($uid, $ownerUid, $cid)
	{
		$resultVo = array('status' => -1);
	    ...
	    $ownerInfo = Hapyfish2_Island_HFC_User::getUser($ownerUid, array('coin' => 1 , 'level' => 1));
	    if ($ownerInfo['level'] < 10) {
            $resultVo['content'] = 'serverWord_1005';
            return array('resultVo' => $resultVo);
	    }
		if($ownerInfo['coin'] <= 0) {
			$resultVo['content'] = 'serverWord_1006';
			return array('resultVo' => $resultVo);
		}
	    $now = time();
	    $ownerCardStatus = Hapyfish2_Island_HFC_User::getCardStatus($ownerUid);
        ...
        $userLevelInfo = Hapyfish2_Island_HFC_User::getUserLevel($uid);
        if ($userLevelInfo['level'] < 10) {
            $resultVo['content'] = 'serverWord_1007';
            return array('resultVo' => $resultVo);
        }
        
        //allow 3 times use
        $blackCountInfo = Hapyfish2_Island_Cache_Counter::getBlackCount($uid);
        if ($blackCountInfo['count'] <= 0) {
        	$resultVo['content'] = 'serverWord_1014';
        	return array('resultVo' => $resultVo);
        }
        
        //allow 3 times used
        $usedBlackCountInfo = Hapyfish2_Island_Cache_Counter::getUsedBlackCount($ownerUid);
        if ($usedBlackCountInfo['count'] <= 0) {
        	$resultVo['content'] = 'serverWord_1008';
        	return array('resultVo' => $resultVo);
        }
        ...
        if($userCoin < 20000) {
        	$resultVo['content'] = 'serverWord_1009';
        	return array('resultVo' => $resultVo);
        }
		$ownerCoin = $ownerInfo['coin'];
		$ownerUpdateMoney = floor($ownerCoin * 0.01);
		
	    //使用者有30%的概率被抓
        $randNum = rand(1,10);
        if($randNum <= 3) {
        	$isCatched = 1;
        	$updateMoney = $ownerUpdateMoney * 2;
        	if($updateMoney >= 20000) {
        		$updateMoney = 20000;
        	}
        }else {
        	$isCatched = 0;
        	$updateMoney = $ownerUpdateMoney;
		...
		$blackCountInfo['count'] = $blackCountInfo['count'] - 1;
		Hapyfish2_Island_Cache_Counter::updateBlackCount($uid, $blackCountInfo);
		
		$usedBlackCountInfo['count'] = $usedBlackCountInfo['count'] - 1;
		Hapyfish2_Island_Cache_Counter::updateUsedBlackCount($ownerUid, $usedBlackCountInfo);
		...
		try {
			...
			if($isCatched == 0) {
				...
				if ($ok) {
					//被使用者随机获得卡片和经验
					$awards = array(
						0	=>	array('cid'=>67441, 'num'=>1, 'exp'=>100)
					);
					$awardCardInfo = Hapyfish2_Island_Cache_BasicInfo::getCardInfo($awards[0]['cid']);
					
					Hapyfish2_Island_HFC_Card::addUserCard($ownerUid, $awards[0]['cid'], $awards[0]['num']);
					Hapyfish2_Island_HFC_User::incUserExp($ownerUid, $awards[0]['exp']);
					
					//add log for ownerUid
					$summary = str_replace('{*coin*}', $updateMoney, LANG_PLATFORM_EXT_TXT_108);
					$summary = str_replace('{*exp*}', $awards[0]['exp'], $summary);
					$summary = str_replace('{*name*}', $awardCardInfo['name'], $summary);
					...
					//add log for uid
					$summary = str_replace('{*coin*}', $updateMoney, LANG_PLATFORM_EXT_TXT_107);
				...
				Hapyfish2_Island_HFC_User::incUserCoin($ownerUid, $updateMoney);
				$ok = Hapyfish2_Island_HFC_User::decUserCoin($uid, $updateMoney);
				if($ok) {
					//add log for ownerUid
					$summary = str_replace('{*coin*}', $updateMoney, LANG_PLATFORM_EXT_TXT_106);
					$minifeed = array('uid' => $ownerUid,
			                          'template_id' => 0,
			                          'actor' => $uid,
			                          'target' => $ownerUid,
					...
					//add log for uid
					$summary = str_replace('{*coin*}', $updateMoney, LANG_PLATFORM_EXT_TXT_105);
					Hapyfish2_Island_Bll_ConsumeLog::coin($uid, $updateMoney, $summary, $now);
					$minifeed = array('uid' => $uid,
			                          'template_id' => 0,
			                          'actor' => $uid,
			                          'target' => $uid,
	 ...
	 * @return array $resultVo
	 */
	public static function sendCoinCard($uid, $ownerUid, $cid)
	{
		$now = time();
		$resultVo = array('status' => -1);
		
        if ($uid == $ownerUid) {
            $resultVo['content'] = 'serverWord_1010';
            return array('resultVo' => $resultVo);
        }
        ...
        $userLevelInfo = Hapyfish2_Island_HFC_User::getUserLevel($uid);
        if ($userLevelInfo['level'] < 10) {
            $resultVo['content'] = 'serverWord_1011';
            return array('resultVo' => $resultVo);
        }
        
        //allow 3 times use
        $sendcoinCountInfo = Hapyfish2_Island_Cache_Counter::getSendCoinCount($uid);
        if ($sendcoinCountInfo['count'] <= 0) {
        	$resultVo['content'] = 'serverWord_1012';
        	return array('resultVo' => $resultVo);
        }
        
	    //allow 3 times used
        $usedsendcoinCountInfo = Hapyfish2_Island_Cache_Counter::getUsedSendCoinCount($$ownerUid);
        if ($usedsendcoinCountInfo['count'] <= 0) {
        	$resultVo['content'] = 'serverWord_1015';
        	return array('resultVo' => $resultVo);
        }
        ...
        $userCoin = $userInfo['coin'];
        if($userCoin <= 0) {
        	$resultVo['content'] = 'serverWord_1013';
        	return array('resultVo' => $resultVo);
        	
        }
		$updateMoney = floor($userCoin * 0.01);     
		if($updateMoney > 10000) {
			$updateMoney = 10000;
		}
		if($updateMoney <= 0) {
		...
		$sendcoinCountInfo['count'] = $sendcoinCountInfo['count'] - 1;
		Hapyfish2_Island_Cache_Counter::updateSendCoinCount($uid, $sendcoinCountInfo);
		
		$usedsendcoinCountInfo['count'] = $usedsendcoinCountInfo['count'] - 1;
		Hapyfish2_Island_Cache_Counter::updateUsedSendCoinCount($ownerUid, $usedsendcoinCountInfo);		

		try {
			require_once(CONFIG_DIR . '/language.php');

			Hapyfish2_Island_HFC_User::incUserCoin($ownerUid, $updateMoney);
			Hapyfish2_Island_HFC_User::incUserExp($uid, 100);
			$ok = Hapyfish2_Island_HFC_User::decUserCoin($uid, $updateMoney);
			if($ok) {
				$uInfo = Hapyfish2_Island_Bll_User::getUserInit($uid);
				$username = $uInfo['name'];
				//add log for ownerUid
				$summary = str_replace('{*coin*}', $updateMoney, LANG_PLATFORM_EXT_TXT_109);
				$summary = str_replace('{*name*}', $username, $summary);
				$minifeed = array('uid' => $ownerUid,
		                          'template_id' => 0,
		                          'actor' => $uid,
		                          'target' => $ownerUid,
	 ...
	 * @return array $resultVo
	 */
	public static function shareMammonCard($uid, $ownerUid, $cid)
	{
		$resultVo = array('status' => -1);
		...
		try {
			...
			$uInfo = Hapyfish2_Island_Bll_User::getUserInit($uid);
			$username = $uInfo['name'];
			$summary = str_replace('{*name*}', $username, LANG_PLATFORM_EXT_TXT_110);
			$minifeed = array('uid' => $ownerUid,
	                          'template_id' => 0,
	                          'actor' => $uid,
	                          'target' => $ownerUid,
