...
class Hapyfish2_Island_Bll_Dock
{
    ...
    public static function getDockShipInfo($ownerUid, $dockPosition, $uid, $now, $mooch = null)
    {
        ...
        if ( $dockPosition['start_visitor_num'] < 1 ) {
        ...
        //get user this position auto info
        //$isAuto = Hapyfish2_Island_Cache_User::isAutoUserDock($uid, $dockPosition['position_id']);
        //$boatItem['isAuto'] = $isAuto;
        $boatItem['unlockShip'] = Hapyfish2_Island_Bll_Dock::readShip($uid, $dockPosition['position_id']);
     ...
     * @return array
     */
    public static function changeShip($uid, $shipId, $pid, $isAuto)
    {
        $resultVo = array('status' => -1);
        ...
        //set user this position is auto
        /*if ( $isAuto == 1 ) {
        	Hapyfish2_Island_Cache_User::updateAutoUserDock($uid, $pid, 1);
        } else {
        	Hapyfish2_Island_Cache_User::updateAutoUserDock($uid, $pid, 0);
        }*/
   ...
   public static function expandPosition($uid)
    ...
    public static function receiveBoat($uid, $pid)
    {
        ...
        $hasShipSuperVisitor = Hapyfish2_Island_Cache_SuperVisitor::hasShipSuperVisitor($uid, $pid);
        if ( $hasShipSuperVisitor ) {
            $superVisitorInfo = Hapyfish2_Island_Bll_SuperVisitor::updateSuperVisitor($uid, $dockPosition['ship_id']);
        }
        ...
        try{
			...
			if ($receiveNum == $dockPosition['remain_visitor_num']) {
		        //get user this position auto info
		        /*$isAuto = Hapyfish2_Island_Cache_User::isAutoUserDock($uid, $pid);
		        if ( $isAuto == 1 ) {*/
	                //get add visitor count by user praise
		        ...
		        /*}
					...
					//clear postition info
		        	$dockPosition['start_visitor_num'] = 0;
		        	$dockPosition['remain_visitor_num'] = 0;
					$dockPosition['speedup'] = 0;
					$dockPosition['speedup_time'] = 0;
					$dockPosition['is_usecard_one'] = 0;
					$dockPosition['receive_time'] = $now;
                ...
                Hapyfish2_Island_Cache_SuperVisitor::updateShipSuperVisitor($uid, $pid, 'Y');
                Hapyfish2_Island_Cache_SuperVisitor::updateMoochShipSuperVisitor($uid, $pid, 'Y');
                ...
                //update this ship,super visitor,小人系统
                Hapyfish2_Island_Cache_SuperVisitor::updateShipSuperVisitor($uid, $pid, 'N');
			}
     ...
     * @return $result array
     */
    public static function stopBoat($uid, $pid)
    {
		$resultVo = array('status' => -1);

		if (!in_array($pid, array(1,2,3,4,5,6,7,8))) {
			return array('result' => $resultVo);
		}
        ...
        //set user this position is not auto
        Hapyfish2_Island_Cache_User::updateAutoUserDock($uid, $pid, 0);
        ...
        try{
			...
			info_log('[stopBoat]:'.$e->getMessage(), 'Hapyfish_Island_Bll_Dock');
			$resultVo['content'] = 'serverWord_110';
            return array('result' => $resultVo);
		}
    ...
    public static function visitorsInvite($uid, $number, $userCurrentIsland, $highcache = true)
    {
		...
		$data['plants'] = is_array($data['plants']) ? $data['plants'] : array();
		$plantInfo1 = $data['plants'];
		
    	$userVo = Hapyfish2_Island_HFC_User::getUserVO($uid);
    	if ( $userCurrentIsland != 1 ) {
			/*if ($highcache) {
				$data1 = Hapyfish2_Island_HFC_Plant::getAllOnIslandFromHighCache($uid, 1);
			} else {
				$data1 = Hapyfish2_Island_HFC_Plant::getOnIsland($uid, 1);
			}*/
				$data1 = Hapyfish2_Island_HFC_Plant::getOnIsland($uid, 1);
			if ( !empty($data1['plants']) ) {
				$data['plants'] = array_merge($data['plants'], $data1['plants']);
			}
    	}
        if ( $userVo['desertIslandState'] == 1 && $userCurrentIsland != 2 ) {
			/*if ($highcache) {
				$data2 = Hapyfish2_Island_HFC_Plant::getAllOnIslandFromHighCache($uid, 2);
			} else {
				$data2 = Hapyfish2_Island_HFC_Plant::getOnIsland($uid, 2);
			}*/
			$data2 = Hapyfish2_Island_HFC_Plant::getOnIsland($uid, 2);
			if ( !empty($data2['plants']) ) {
				$data['plants'] = array_merge($data['plants'], $data2['plants']);
			}
        }
        if ( $userVo['hawaiiIslandState'] == 1 && $userCurrentIsland != 3 ) {
			/*if ($highcache) {
				$data3 = Hapyfish2_Island_HFC_Plant::getAllOnIslandFromHighCache($uid, 3);
			} else {
				$data3 = Hapyfish2_Island_HFC_Plant::getOnIsland($uid, 3);
			}*/
        	$data3 = Hapyfish2_Island_HFC_Plant::getOnIsland($uid, 3);
			if ( !empty($data3['plants']) ) {
				$data['plants'] = array_merge($data['plants'], $data3['plants']);
			}
        }
        if ( $userVo['iceLandState'] == 1 && $userCurrentIsland != 4 ) {
			/*if ($highcache) {
				$data4 = Hapyfish2_Island_HFC_Plant::getAllOnIslandFromHighCache($uid, 4);
			} else {
				$data4 = Hapyfish2_Island_HFC_Plant::getOnIsland($uid, 4);
			}*/
			$data4 = Hapyfish2_Island_HFC_Plant::getOnIsland($uid, 4);
			if ( !empty($data4['plants']) ) {
				$data['plants'] = array_merge($data['plants'], $data4['plants']);
			}
        }
		
		$plantInfoAllHave = $data['plants'];
		if ( empty($plantInfo1) ) {
			$plantInfo1 = array();
		}
		if ( empty($plantInfoAllHave) ) {
			$plantInfoAllHave = array();
		}
		
		$cids = array();
    	foreach ($plantInfoAllHave as $item) {
        ...
        foreach ($plantInfo2 as $key2 => $value2) {
        	foreach ($plantInfoAllHave as $value1) {
        ...
        foreach ($plantInfo3 as $key3 => $value3) {
        	foreach ($plantInfoAllHave as $value11) {
				...
				if ($value_num > 0) {
					...
					if ($value['wait_visitor_num'] == 0 && $value['start_deposit'] == 0) {
					    ...
					    if ($now > $defenseCardTime) {
                            $rand = rand(1, 100);
                            $isMiracle = false;
                            if(isset($value['act_name'])){
                            	$str = explode('_', $value['act_name']);
                            	if($str[0] == 'miracle'){
                            		$isMiracle = true;
                            	}
                            }
                            if ( $rand < 31 && !$isMiracle) {
                                $value['event'] = 1;
                            }
                        }
					}
    ...
    public static function mooch($uid, $ownerUid, $positionId)
    {
        ...
        //check this ship ,can mooch super visitor,小人系统
        $canMoochSv = Hapyfish2_Island_Cache_SuperVisitor::canMoochSuperVisitor($ownerUid, $positionId);
        if ( $canMoochSv ) {
            $moochSuperVisitorInfo = Hapyfish2_Island_Bll_SuperVisitor::updateMoochSuperVisitor($uid, $ownerUid, $dockPosition['ship_id'], $positionId);
        }
        ...
        try {
            ...
            //小人系统
            Hapyfish2_Island_Cache_SuperVisitor::updateMoochShipSuperVisitor($ownerUid, $positionId, 'N');
            $result['status'] = 1;
        ...
        if ( $canMoochSv ) {
            $resultVo['spVisitors'] = $moochSuperVisitorInfo;
        }
        return $resultVo;
    }
