...
//				err_log($e->getMessage());
				info_log($e->getMessage(),'getMulti');
				return null;
			}
		}
    ...
    public static function incUserGold($uid, $goldChange)
    {
    	$toSendGold = array('gold' => $goldChange, 'type' => 0, 'time' => time());
    	Hapyfish2_Island_Bll_Gold::add($uid, $toSendGold);
    }    
    ...
    public static function getUserLoginInfo($uid)
    {
		...
		$starKey = 'i:u:star:login:' . $uid;
    	$starcache = Hapyfish2_Cache_Factory::getHFC($uid);
    	$starVa = $starcache->get($starKey);
 	
    	if(!$starVa || $starVa == 1 ) {
    		$starcache->update($starKey, 2);
    ...
    public static function delTitle($uid, $title, $savedb = false)
    {
    	$userTitle = self::getUserTitle($uid);
    	if (!$userTitle) {
    		return false;
        ...
        $newTitle = $userTitle['title'];
    	if($userTitle['title'] == '100') {
    		$newTitle = 0;
    	}    	
    	$newUserTitle = array('title' => $newTitle, 'title_list' => join(',', $tmp));
