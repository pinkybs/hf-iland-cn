...
class Hapyfish2_Island_Stat_Dal_DailyPayment
{
    ...
    {
    	//return 'island_user_payment_' . $id;
    ...
    public function getDailyPaymentStatUserCnt($uid, $id, $startDate, $endDate)
    {
    	$db = Hapyfish2_Db_Factory::getDB($uid);
        $rdb = $db['r'];

    	$tbname = $this->getPayTableName($id);
    	$sql = "SELECT COUNT(1) FROM (SELECT count(1) FROM $tbname
    			WHERE order_time>=$startDate AND order_time<$endDate AND status=1 GROUP BY uid) AS u ";
