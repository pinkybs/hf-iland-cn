...
info_log(date('Ymd',$startDate).'********', 'stat_daily_pay');
			foreach ($aryDb as $db=>$dbUId) {
				//for ($i=0; $i<10; $i++) {
					...
					$payUserCount = $dal->getDailyPaymentStatUserCnt($dbUId, 0, $startDate, $endDate);
					$row['userCnt'] = $payUserCount;
					$rowStat = $dal->getRow($logDate);
					if (empty($rowStat)) {
						$dal->insert(array('log_time'=>$logDate, 'amount'=>$row['amount'], 'gold'=>$row['gold'], 'trans_count'=>$row['cnt'], 'user_count'=>$row['userCnt']));
					}
					else {
						$dal->updateByMultipleField($logDate, array('gold'=>$row['gold'], 'amount'=>$row['amount'], 'trans_count'=>$row['cnt'], 'user_count'=>$row['userCnt']));
					}
