...
class Hapyfish2_Island_Dal_Plant
{
    ...
    public function init($uid)
    {
        ...
        $sql = "INSERT INTO $tbname(uid,id,cid,level,item_id,item_type,x,y,z,mirro,status,buy_time,start_pay_time,start_deposit,deposit)
             ...
             (:uid, 3, 632,   2, 6,   32, 7,  7,  0, 0, 1, $time, 0, 0, 0),
    ...
    public function clearDiy($uid, $islandId)
    {
        ...
        $sql = "UPDATE $tbname SET status=0,start_deposit=0,deposit=0,event=0,wait_visitor_num=0 WHERE uid=:uid AND status=:islandId";
	...
	public function getOneId($uid, $cid)
	{
		...
		$sql = "SELECT id FROM $tbname WHERE uid=:uid AND cid=:cid";
	...
	public function getOneIdByItemid($uid, $itemId)
	{
		$tbname = $this->getTableName($uid);
		
		$sql = "SELECT id FROM $tbname WHERE uid=:uid AND item_id=:item_id";

		$db = Hapyfish2_Db_Factory::getDB($uid);
        $rdb = $db['r'];

        return $rdb->fetchCol($sql, array('uid' => $uid, 'item_id' => $itemId));
	}
	...
	public function getUidByCid($id, $cid)
	{
		$tb = $this->getTableName($id);
		
		$sql = "SELECT uid FROM $tb WHERE cid=:cid";
