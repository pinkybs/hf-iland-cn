...
class Hapyfish2_Island_Dal_Dock
{
    ...
    public function init($uid)
    {
        ...
        $time = $now - 1200;
    	$tbname = $this->getTableName($uid);
        $sql = "INSERT INTO $tbname(uid, position_id, receive_time, start_visitor_num) VALUES(:uid, 1, $time, 6),(:uid, 2, $time, 6),(:uid, 3, $time, 6)";
    ...
    public function getDockData($uid, $info)
    {
		$tbname = $this->getTableName($uid);
    	$sql = "SELECT position_id FROM $tbname WHERE uid=:uid AND position_id=:position_id";
    ...
    public function updateDockData($uid, $info)
    {
		$tbname = $this->getTableName($uid);
        $sql = "UPDATE $tbname SET unlock_ship_ids=:unlock_ship_ids WHERE uid=:uid AND position_id=:position_id";
        ...
        $wdb->query($sql, array('uid' => $uid, 'position_id' => $info['position_id'], 'unlock_ship_ids' => $info['unlock_ship_ids']));
    }
