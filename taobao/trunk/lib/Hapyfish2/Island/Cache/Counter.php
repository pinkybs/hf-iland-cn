...
class Hapyfish2_Island_Cache_Counter
{
    ...
    /**
     * update by hdf 2011-12-09
     * black use 3 times one day 
     */
	public static function getBlackCount($uid)
	{
		$today = date('Ymd');
	...
	public static function updateBlackCount($uid, $info)
	{
		$key = 'i:u:blackcntdly:' . $uid;
		$cache = Hapyfish2_Cache_Factory::getMC($uid);
		$data = array($info['today'], $info['count']);
	...
	public static function getUsedBlackCount($uid)
	{
		$today = date('Ymd');
	...
	public static function updateUsedBlackCount($uid, $info)
	{
		$key = 'i:u:usedblackcntdly:' . $uid;
		$cache = Hapyfish2_Cache_Factory::getMC($uid);
		$data = array($info['today'], $info['count']);
	...
	public static function getSendCoinCount($uid)
	{
		$today = date('Ymd');
    	$key = 'i:u:sendcoincntdly:' . $uid;
		$cache = Hapyfish2_Cache_Factory::getMC($uid);
		$data= $cache->get($key);
		if ($data === false) {
			$data = array($today, 3);
	...
	public static function updateSendCoinCount($uid, $info)
	{
		$key = 'i:u:sendcoincntdly:' . $uid;
		$cache = Hapyfish2_Cache_Factory::getMC($uid);
		$data = array($info['today'], $info['count']);
	...
	public static function getUsedSendCoinCount($uid)
	{
		$today = date('Ymd');
    	$key = 'i:u:usedsendcoincntdly:' . $uid;
		$cache = Hapyfish2_Cache_Factory::getMC($uid);
		$data= $cache->get($key);
		if ($data === false) {
			$data = array($today, 3);
	...
	public static function updateUsedSendCoinCount($uid, $info)
	{
		$key = 'i:u:usedsendcoincntdly:' . $uid;
		$cache = Hapyfish2_Cache_Factory::getMC($uid);
		$data = array($info['today'], $info['count']);
