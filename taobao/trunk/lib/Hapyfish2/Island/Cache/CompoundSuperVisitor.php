...
class Hapyfish2_Island_Cache_CompoundSuperVisitor
{
    ...
    public static function hasShipSuperVisitor($uid)
    {
        $key = 'i:u:compound:svs:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getMC($uid);
		$data = $cache->get($key);
		if ( $data != 'N') {
			return true;
    ...
    public static function updateShipSuperVisitor($uid, $data)
    {
        $key = 'i:u:compound:svs:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getMC($uid);
		$cache->set($key, $data);
    ...
    {
        $key = 'i:u:compound:todayrenum:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getMC($uid);
		$data = $cache->get($key);
		if ( $data === false ) {
			return 1;
    ...
    {
        $key = 'i:u:compound:moochsvs:' . $uid . ':' . $postionId;
        $cache = Hapyfish2_Cache_Factory::getMC($uid);
		$data = $cache->get($key);
		if ( $data != 'N') {
			return true;
    ...
    {
        $key = 'i:u:compound:coltn:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getHFC($uid);
		$data = $cache->get($key);
        if ($data === false) {
        	try {
	            $dalSuperVisitor = Hapyfish2_Island_Dal_CompoundSuperVisitor::getDefaultInstance();
    ...
    {
        $key = 'i:u:compound:todaycoltn:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getHFC($uid);
		$data = $cache->get($key);  
        if ($data === false) {
            $data = array();
     ...
     * 随机小人奖励
     */
    public static function getSvGiftRandArray()
    {
        $list = self::getSvGiftList();
        $randArray = array();
        for ( $i=0,$iCount=count($list); $i<$iCount; $i++ ) {
     ...
     * 小人奖励列表
     */
    public static function getSvGiftList()
    {
        $db = Hapyfish2_Island_Dal_BasicInfo::getDefaultInstance();
        $list = $db->getCompoundSvGiftList();
        if ($list) {
            $key = 'i:compound:svgiftlist';
    ...
    public static function getSvGiftById($id)
    {
        $list = self::getSvGiftList();
        if (isset($list[$id])) {
            return $list[$id];
