...
class Hapyfish2_Island_Event_Bll_StarfishSale
{
    ...
    public static function Exchange($uid, $cid)
    {
        ...
        if($userStarFish < $detail['price']){
        	$result['result'] = array('status' => -1, 'content' => '对不起您的海星不够');
            return $result;
     	}
		$now = time();
		$type = substr($cid, -2, 2);
		if($type == 61){
			...
			$ok = Hapyfish2_Island_Bll_Compound::addMaterial($uid, $cid, $detail['number']);
		}else{
			...
			$ok = $compensation->sendOne($uid, '恭喜你成功兑换');
		}
		if($ok){
		    $summary = '兑换' . $cinfo['name'] . '消耗'.$detail['price'].'个海星';
