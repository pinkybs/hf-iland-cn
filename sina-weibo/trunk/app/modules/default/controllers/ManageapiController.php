...
class ManageapiController extends Zend_Controller_Action
{
    ...
    public function addstarfishAction(){
    	...
    	$title = '恭喜你开心论坛奖励<font color="#9F01A0">'.$num.'个海星</font>';
			$feed = array(
				'uid' => $uid,
				'actor' => $uid,
				'target' => $uid,
...
//				'title' => array('cardName' => '加速卡II'),
    ...
    public function clearusertaskAction()
    {
     $uid=$this->_request->getParam('uid');
      $key = 'i:u:alltask:' . $uid;
      ...
      $key2 = 'i:u:title:'.$uid;
      $cache2 = Hapyfish2_Cache_Factory::getHFC($uid);
      $cache2->delete($key2) ;
      echo "OK";
      exit;
    }
		...
		if($gold > 0){
			...
			if($uidlist) {
				foreach($uidlist as $val) {
					$logger->report('801', array($val, $gold, 0));
				}
			}
			//end			
		}
