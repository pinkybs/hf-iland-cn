...
class SinaWeibo_Weiyouxi
{
     ...
     */
    protected $session = array('sessionKey' => null, 'userId' => null, 'create' => null,
    'expire' => null);
    ...
    {
        $this->isCurlBegun = false;
        $this->source = $source;
        $this->secret = $secret;
        $this->sessionKey = empty($_GET[self::PREFIX_PARAM . 'session_key']) ? null : $_GET[self::PREFIX_PARAM .
         'session_key'];
        $this->signature = empty($_GET[self::PREFIX_PARAM . 'signature']) ? null : $_GET[self::PREFIX_PARAM .
         'signature'];
    ...
    public function checkSessionKey()
    {
        ...
        $this->session['create'] = empty($_GET[self::PREFIX_PARAM . 'create']) ? null : $_GET[self::PREFIX_PARAM .
         'create'];
        $this->session['expire'] = empty($_GET[self::PREFIX_PARAM . 'expire']) ? $expire : $_GET[self::PREFIX_PARAM .
         'expire'];
    }
    ...
    {
        return json_decode(
        $this->http($this->apiUrl . $api, $this->buildQueryParamStr($data), 1), true);
    }
    ...
    {
        return json_decode(
        $this->http($this->apiUrl . $api, $this->buildQueryParamStr($data), 0), true);
    }

    public function buildQueryParamStr(&$data)
    {
        $timestamp = microtime(true);
        $params = array('source' => $this->source, 'timestamp' => $timestamp);
    ...
    {
        if (is_array($input)) {
            return array_map(array('SinaWeibo_Weiyouxi', 'urlencodeRfc3986'), $input);
        }
...
//info_log($url.'?'.$dataStr, 'aa');
        ...
        curl_close($ch);
        //info_log((getmicrotime()-$startTm).','.$url, 'stat_wbapi_call_time');
        return $response;
    }
    ...
    public function getCurlBegunStatus()
    {
        return $this->isCurlBegun;
    }
}