...
class Hapyfish2_Platform_Cache_User
{
    ...
    public static function updateVerified($uid, $data)
    {
		...
		try {
        	$dalUser = Hapyfish2_Platform_Dal_User::getDefaultInstance();
        	$dalUser->update($uid, array('verified' => $data));
        } catch (Exception $e) {
    ...
    public static function updateHFCUser($uid)
    {
        try {
            $dalUser = Hapyfish2_Platform_Dal_User::getDefaultInstance();
            $rst = $dalUser->getInfo($uid);
            if ($rst) {
                $hfcKey = 'p:u:' . $uid;
                $hfcCache = Hapyfish2_Cache_Factory::getHFC($uid);
            	$hfcCache->set($hfcKey, $rst);
            	return true;
            }
        } catch (Exception $e) {
            err_log('Hapyfish2_Platform_Cache_User::updateHFCUser Err'. $e->getMessage());
        }
        return false;
    }
