...
class Hapyfish2_Island_Bll_Compensation
{
	...
	public function sendOne($uid, $feedPrefix, $sendFeed = true)
	{
		...
		$desp = array();
		if ($this->_coin > 0) {
			$ok = Hapyfish2_Island_HFC_User::incUserCoin($uid, $this->_coin, true, 1);
			if ($ok) {
		...
		if ($this->_starFish > 0) {
			...
			if ($ok) {
				$desp[] = $this->_starFish . '海星';
			}
		}
		if($this->_fragment > 0){
			$userFragment = Hapyfish2_Island_Bll_Fragments::getUserFragments($uid);
			$fnum = isset($userFragment['fragment_num'])?$userFragment['fragment_num']:0;
			$fragment = $this->_fragment + $fnum;
			Hapyfish2_Island_Bll_Fragments::updateFragments($uid, array('fragment_num'=> $fragment));
			$desp[] = $this->_fragment . '碎片';
		}
