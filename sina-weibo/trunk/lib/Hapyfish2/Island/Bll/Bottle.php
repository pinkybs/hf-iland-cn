...
class Hapyfish2_Island_Bll_Bottle
{
	...
	public static function click($idx, $type, $uid, $betNum)
	{
		...
		// 黄钻用户宝石优惠
		$normal_exchange= array('3'=>5, '4'=>4, '5'=>40);
		...
		// 获得玩家好友帮助
		$friendHelp = Hapyfish2_Island_Cache_BottleFriendHelp::getByUid($uid);
		...
		if ($type == '1') {
			...
			if (count(explode(',', $friendHelp['fid'])) < 5 ) {
				$result['content'] = 'serverWord_110';
            	return array('result'=>$result);
			}
			if ($friendHelp['lasttime'] && time() < $friendHelp['lasttime']+(3600*8)) {
				$result['content'] = 'serverWord_110';
            	return array('result'=>$result);
			}
		}
		...
		$list = (in_array($type, array(1,3,6)) || ($type==2&&$betNum==1) ? array($list) : $list);
		...
		if ($type == 1) {
			...
			$ok2 = self::itemstouser($list, $uid);
			if ($ok2['tf']) {
				Hapyfish2_Island_Cache_BottleFriendHelp::reset($uid);
			}
		...
		if ($ok2['tf']) {
			...
			*/
			...
			$result['coinChange'] = $ok2['coin'];
			$result['goldChange'] = $result['goldChange'] + $ok2['gold'];
	...
	public static function itemstouser($list, $uid)
	{
		...
		return array('coin'=>$coin, 'gold'=>$gold, 'starfish'=>$starfish, 'tf'=>$com->sendOne($uid,''));
