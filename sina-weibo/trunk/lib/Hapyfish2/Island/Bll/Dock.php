...
class Hapyfish2_Island_Bll_Dock
{
    ...
    public static function unlockShip($uid, $shipId, $pid, $priceType)
    {
    	...
    	try {
			...
			if ($levelUp['feed']) {
            	...
            	$feed = Hapyfish2_Island_Bll_Activity::send('BOAT_LEVEL_UP', $uid);
            	$sendGift = 'unlockShip';
            	$ukey = 'i:u:sf:u:'.$uid;
        ...
        return array('result'=>$resultVo, 'feed'=>$feed, 'sendGift'=>$sendGift);
    }
   ...
   public static function expandPosition($uid)
    ...
    public static function receiveBoat($uid, $pid)
    {
        ...
        try{
			...
			if ($receiveNum == $dockPosition['remain_visitor_num']) {
		        ...
		        //update this ship,super visitor
			...
			Hapyfish2_Island_HFC_User::incUserExp($uid, $addExp);
            $visitorsAry = self::visitorsInvite($uid, $receiveNum, true);
     ...
     * @return array
     */
    public static function visitorsInvite($uid, $number, $highcache = true)
    {
		$visitorsAry = array();

		if ($highcache) {
			$data = Hapyfish2_Island_HFC_Plant::getAllOnIslandFromHighCache($uid);
		} else {
    ...
    public static function mooch($uid, $ownerUid, $positionId)
    {
        ...
        //check this ship ,can mooch super visitor
        $canMoochSv = Hapyfish2_Island_Cache_SuperVisitor::canMoochSuperVisitor($ownerUid, $positionId);
        if ( $canMoochSv ) {
        	$moochSuperVisitorInfo = Hapyfish2_Island_Bll_SuperVisitor::updateMoochSuperVisitor($uid, $ownerUid, $positionId);
        }
