...
class Hapyfish2_Island_Bll_Task
{
    ...
    public static function readTask($uid)
    {
    	...
    	foreach ($achievementTask as $id => $task) {
    		if(in_array($id, array(3099, 3100))) {
				continue;
			}
        ...
        //add by zx for sinaweibo achieve system 20110614
        Hapyfish2_Platform_Bll_WeiboAchieve::checkAchieveAll($uid, $userAchievementTask);
    ...
    public static function finishTask($uid, $taskId)
    {
        ...
        if (is_file(LIB_DIR . '/' . $implFile)) {
            ...
            if ($result['status'] == 1) {
	        	...
	        	//compute finish task count
		        try {
		        	//add log
					$logger = Hapyfish2_Util_Log::getInstance();
					$logger->report('3004', array($taskId));
		        } catch (Exception $e) {
		        }
            }
    ...
    public static function checkTask($uid, $taskId)
    {
        ...
        if ( $result['status'] == 1 ) {
	        ...
	        try {
	        	//add log
				$logger = Hapyfish2_Util_Log::getInstance();
				$logger->report('3004', array($finishTaskId));
	        } catch (Exception $e) {
	        }
        }
        
        //add by zx for sinaweibo achieve system 20110614
        Hapyfish2_Platform_Bll_WeiboAchieve::checkAchieveId($uid, $result['finishTaskId']);
