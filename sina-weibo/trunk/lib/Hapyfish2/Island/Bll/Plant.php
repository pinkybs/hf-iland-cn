...
{
	public static function getAllOnIsland($ownerUid, $uid, $highcache = false)
    {
		$data = Hapyfish2_Island_HFC_Plant::getOnIsland($ownerUid, $highcache);
		$plants = array();
		$visitorNum  = 0;
		...
		if ($data) {
			...
			Hapyfish2_Island_Bll_PlantStatus::outIslandPeople($ownerUid, $data['plants']);
    ...
    public static function getAllOnIslandNoMooch($uid, $savehighcache = true, $checkout = false)
    {
		$data = Hapyfish2_Island_HFC_Plant::getOnIsland($uid, $savehighcache);
		$plants = array();
		$visitorNum  = 0;
    ...
    public static function managePlant($uid, $itemId, $eventType, $ownerUid)
    {
        ...
        if ( $userPlant['event'] != 2 ) {
	        ...
	        if ( $now - $userPlant['start_pay_time'] + $userPlant['delay_time'] < $plantInfo['pay_time'] * 0.6 ) {
	        	$result['content'] = 'serverWord_146';
            	/*$result['now'] = $now;
            	$result['start_pay_time'] = $userPlant['start_pay_time'];
            	$result['delay_time'] = $userPlant['delay_time'];
            	$result['pay_time'] = $plantInfo['pay_time'] * 0.6;
            	$result['pay_time1'] = $now - $userPlant['start_pay_time'] + $userPlant['delay_time'];*/
	            return array('resultVo' => $result);
	        }
	...
	public static function harvestAllPlant($uid)
	{
		...
		if ($data['plants']) {
			Hapyfish2_Island_Bll_PlantStatus::outIslandPeople($uid, $data['plants']);
