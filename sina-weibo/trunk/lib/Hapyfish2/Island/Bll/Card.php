...
class Hapyfish2_Island_Bll_Card
{
	...
	public static function plunderCard($uid, $ownerUid, $cid)
	{
		...
		try {
			Hapyfish2_Island_HFC_User::incUserExpAndCoin($uid, $cardInfo['add_exp'], $updateMoney, true, 1);
...
//        }
	...
	public static function clearDiy($uid, $type)
	{
		...
		if ( $type == 2 ) {
			...
			Hapyfish2_Island_Bll_Island::reload($uid, true);
		}
		else {
			$cache = Hapyfish2_Cache_Factory::getHFC($uid);
	    	...
	    	if ( $userCurrentIsland > 1 ) {
	        	$key = 'i:u:pltids:onisl:' . $uid . ':' . $userCurrentIsland;
	    	}
	    	else {
