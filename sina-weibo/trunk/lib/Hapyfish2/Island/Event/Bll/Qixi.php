...
class Hapyfish2_Island_Event_Bll_Qixi
{
	...
	const TXT007 = '对不起您没改该建筑不能抽奖';
	const TXT012 = '恭喜你获得了七夕鹊桥礼物';
	const TXT013 = '获得七夕鹊桥礼物';
	...
	public static function getGift($uid)
	{
		...
		if($switch){
			$rsp = array ();
			$rsp ['resultVo'] ['status'] = - 1;
			$rsp ['resultVo'] ['content'] = '您已领取过，或活动已过期！';
			return $rsp;
		}
    	...
    	if( $i == 3 ) {
    		...
    		$rsp = array ();
			$rsp ['resultVo'] ['status'] = - 1;
			$rsp ['resultVo'] ['content'] = '没有收集齐，不能领取！';
    	}
		return $rsp;
	}
