...
class Hapyfish2_Island_Event_Bll_Tkhash
{
	...
	public static function getGift($uid)
	{
		...
		if(!$switch){
			$rsp = array ();
			$rsp ['result'] ['status'] = - 1;
			$rsp ['result'] ['content'] = '活动暂未开放，敬请期待！';
			return $rsp;
		}

		$rsp = array ();
		$rsp ['result'] ['status'] = - 1;
		$rsp ['result'] ['content'] = '您已领取过奖励，快去仓库看看吧！';
		...
		if( !$val ) {
	    	...
	    	if( $i == 7 ) {
				...
				if($ok) {
					$title = '恭喜你获得收集物品<font color="#FF0000">' . $name . '</font>，快去仓库看看吧！';
	...
	public static function collectionTask($uid)
	{
		...
		if(!$switch){
			...
			$result ['result'] ['content'] = '活动暂未开放，敬请期待！';
			return $result;
		}
		...
		$rsp ['data'] [] = array ('window' => 'window6', 'cid' => '', 'name' => '', 'tip' => '', 'haveGet' => false );
		$rsp ['data'] [] = array ('window' => 'window7', 'cid' => '', 'name' => '', 'tip' => '', 'haveGet' => false );
		$rsp ['data'] [] = array ('window' => 'window8', 'cid' => '' );
	...
	public static function getval( $key )
	{
        ...
        if( empty($data)) {
        	$hash = Hapyfish2_Island_Event_Dal_Tkhash::getDefaultInstance();
        	$val = $hash->getval( $key );
        	if( $val ) {
