...
class Hapyfish2_Island_Cache_BasicInfo
{
	...
	public static function getIslandLevelInfoByUserLevel($userLevel)
	{
		$islandLevelList = self::getIslandLevelList();
		$islandLevel = 0;
		foreach ($islandLevelList as $item) {
			if ($item['need_user_level'] <= $userLevel) {
	...
	public static function loadSVDemandList()
	{
		$db = Hapyfish2_Island_Dal_BasicInfo::getDefaultInstance();
		$list = $db->getSVDemandList();
		info_log('list:'.json_encode($list), 'demand');
		if ($list) {
			$key = 'island:svdemandlist';
			$cache = self::getBasicMC();
			$cache->set($key, $list);
	...
	public static function getSVDemandListById($demandId)
	{
		$list = self::getSVDemandList();
		if (isset($list[$demandId])) {
			return $list[$demandId];
		}
		return null;
	}
	...
	public static function loadSVisitorList()
	{
		$db = Hapyfish2_Island_Dal_BasicInfo::getDefaultInstance();
		$list = $db->getSVisitorList();
		if ($list) {
			$key = 'island:svisitorlist';
			$cache = self::getBasicMC();
			$cache->set($key, $list);
	...
	public static function getCollectionById($cid)
	{
		$list = self::getCollectionList();
		if (isset($list[$cid])) {
			return $list[$cid];
		}
		return null;
	...
	public static function loadCollectionList()
	{
		$db = Hapyfish2_Island_Dal_BasicInfo::getDefaultInstance();
		$list = $db->getCollectionList();
		if ($list) {
			$key = 'island:collectionlist';
			$cache = self::getBasicMC();
			$cache->set($key, $list);
	...
	public static function getCollectionGroups()
	{
		...
		if (!$list) {
			...
			$list = $cache->get($key);
			if (!$list) {
				$list = self::loadCollectionGroups();
			}
			if ($list) {
				$localcache->set($key, $list);
			}
	...
	public static function loadCollectionGroups()
	{
		$db = Hapyfish2_Island_Dal_BasicInfo::getDefaultInstance();
		$list = $db->getCollectionGroups();
		if ($list) {
			$key = 'island:collectiongroups';
			$cache = self::getBasicMC();
			$cache->set($key, $list);
	...
	public static function getCollectionGroupById($groupId)
	{
		$list = self::getCollectionGroups();
	...
	public static function getCollectionRandArray()
	{
		$key = 'i:colnrandary';
		$localcache = Hapyfish2_Cache_LocalCache::getInstance();
		...
		if (!$list) {
			...
			$list = $cache->get($key);
			if (!$list) {
				$list = self::loadCollectionRandArray();
			}
			if ($list) {
				$localcache->set($key, $list);
			}
    ...
    public static function loadCollectionRandArray()
    {
		$list = Hapyfish2_Island_Cache_BasicInfo::getCollectionList();
		$randArray = array();
		for ( $i=1,$iCount=count($list); $i<=$iCount; $i++ ) {
			for ( $j=0; $j<$list[$i]['odds']; $j++ ) {
				$randArray[] = $list[$i]['cid'];
			}
		}
		for ( $k = 0; $k < 50; $k++ ) {
			$randArray[] = 0;
