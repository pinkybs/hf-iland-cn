...
class Hapyfish2_Island_Bll_PlantStatus
{
	 ...
	 * @param integer $uid
	 */
	public static function outIslandPeople($uid, &$plants)
	{
		if (!$plants) {
			return;
		}
		
		$lastOutIslandPeopleTime = Hapyfish2_Island_Cache_PlantStatus::getLastOutIslandPeopleTime($uid);
		$now = time();
		...
		$canDo = Hapyfish2_Island_Cache_PlantStatus::canOutIslandPeople($uid);
		if (!$canDo) {
			return;
		}
		
		Hapyfish2_Island_Cache_PlantStatus::updateLastOutIslandPeopleTime($uid, $now);
    ...
    public static function outPlantPeople($uid, &$plant, $now)
    {
    	...
    	$lastOutIslandPeopleTime = Hapyfish2_Island_Cache_PlantStatus::getLastOutIslandPeopleTime($uid);
    	$lastOutPlantPeopleTime = Hapyfish2_Island_Cache_PlantStatus::getLastOutPlantPeopleTime($uid, $itemId);
