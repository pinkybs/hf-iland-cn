...
class Hapyfish2_Island_Bll_LevelUpLog
{
	public static function add($uid, $from_level, $to_level)
	{
		...
		try {
			$dalLog = Hapyfish2_Island_Dal_LevelUpLog::getDefaultInstance();
	...
	public static function check($uid, $level, $time)
	{
		if ($level > 10) {
			...
			if ($all) {
				...
				if (!$valid) {
					Hapyfish2_Island_Bll_Block::add($uid, 2, 2);
				}
			}
		}
	}
