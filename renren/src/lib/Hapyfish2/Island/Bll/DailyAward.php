...
class Hapyfish2_Island_Bll_DailyAward
{
    ...
    public static function getAwards($uid)
    {
		...
		$rest = Xiaonei_Renren::getInstance();
    	...
    	try {
	        ...
	        $dailyReward['items'] = self::_getTodayAwardsItem($result['awardNum'], $loginInfo['active_login_count']);
    ...
    public static function gainAwards($uid)
    {
		...
		if ($dailyReward && $dailyReward['items'] && $dailyReward['date'] == $nowDate && empty($dailyReward['gained'])) {
			...
			if ($starfishChange){
				$ok = Hapyfish2_Island_Bll_StarFish::add($uid, $starfishChange, '登录翻牌');
				$title = '获得每日翻贝壳奖励:'.$starfishChange.'个海星';
				if( $ok ){
					$feed = array(
					'uid' => $uid,
					'actor' => $uid,
	 ...
	 * @return array
	 */
    private static function _getTodayAwardsItem($cntChance=1, $day)
    {
    	$retItem = array();
		//get rand item basic
		$aryItem = array();
		...
		if (1 == $cntChance) {
			...
			if($day >=5){
				$retItem[] = $aryItem[$goldkey];
	        	unset($aryRandOdds[$goldkey]);
	        	$gainKey2 = self::_randomKeyForOdds($aryRandOdds);
	        	$retItem[] = $aryItem[$gainKey2];
	        	unset($aryRandOdds[$gainKey2]);
