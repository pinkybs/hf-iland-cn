...
class Hapyfish2_Island_Bll_Task
{
    ...
    public static function readTask($uid)
    {
    	...
    	foreach ($achievementTask as $id => $task) {
    		if(in_array($id, array(3068, 3069, 3070, 3083, 3084, 3085))) {
				continue;
			}
			...
			if($id == 3086) {
				...
				$allBuildingCid = $dalBuilding->getAllCid($uid);

				foreach($allBuildingCid as $hasCid) {
					$hasThidCid[] = $hasCid['cid'];
				}

				if(in_array(55421, $hasThidCid)) {
					$hasThid = 1;
				}

				if($hasThid == 1) {
					Hapyfish2_Island_HFC_User::gainTitle($uid, 82);
				}
			}
			else {
