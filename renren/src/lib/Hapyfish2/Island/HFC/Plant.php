...
{
	public static function getOnIsland($uid, $savehighcache = false)
    {
        ...
        if ($empty) {
        	try {
	            ...
	            $result = $dalPlant->getOnIsland($uid);
	            if ($result) {
	            	$data = array();
    ...
    public static function getAllOnIslandFromHighCache($uid)
    {
    	$key = 'island:allplantonisland:' . $uid;
    	$hc = Hapyfish2_Cache_HighCache::getInstance();
    	return $hc->get($key);
    ...
    public static function getOne($uid, $id, $status = 1)
    {
    	...
    	if ($item === false) {
    		try {
	    		$dalPlant = Hapyfish2_Island_Dal_Plant::getDefaultInstance();
	    		$item = $dalPlant->getOneOnIsland($uid, $id);
	    		if ($item) {
	    			$cache->add($key, $item);
	    		} else {
	    			return null;
    ...
    public static function addOne($uid, $plant)
    {
    	$result = false;
    	try {
    ...
    public static function delOne($uid, $id, $status)
    {
    	$result = false;
    	try {
    		$dalPlant = Hapyfish2_Island_Dal_Plant::getDefaultInstance();
    ...
    public static function upgradeCoordinate($uid, $step = 1)
    {
		$ids = Hapyfish2_Island_Cache_Plant::getOnIslandIds($uid);
	    if (!$ids) {
        	return false;
        ...
        if (!$empty) {
        	try {
	            $dalPlant = Hapyfish2_Island_Dal_Plant::getDefaultInstance();
	            $dalPlant->upgradeCoordinate($uid, $step);
