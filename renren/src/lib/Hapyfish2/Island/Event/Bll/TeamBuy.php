...
{
	const TXT001 = '团购活动已结束,请期待下一期团购';
	const TXT002 = '你已经加入此次团购,请等待购买';
	const TXT003 = '团购活动已结束,请期待下一期团购';
	const TXT004 = '团购活动报名时间已结束,请期待下一期团购';
	const TXT005 = '你已经购买过团购物品!';
	...
	public static function getPriceType($joinNum, $min, $max, $info)
	{
		...
		if($max[1] == 2) {
			...
			if($min[1] == 2) {
				...
				$max_price = array('price_type' => $price_type, 'price' => $max[0]);
				$min_price = array('price_type' => $price_type, 'price' => $min[0]);
				...
				if($joinNum < $info['bec_num']) {
					...
					$max_price = array('price_type' => $max[1], 'price' => $max[0]);
					$min_price = array('price_type' => $min[1], 'price' => $min[0]);
			...
			if($joinNum < $info['max_num']) {
				...
				$now_price = array('price_type' => $price_type, 'price' => $min[0]);
				$dec_price = array('price_type' => $price_type, 'price' => $price_Dec);
			}
			$max_price = array('price_type' => $price_type, 'price' => $max[0]);
			$min_price = array('price_type' => $price_type, 'price' => $min[0]);
