...
class Hapyfish2_Island_Dal_GiftPackage
{
	...
	public function getGiftId($uid,$pack)
	{
		...
		$sql = "SELECT pid FROM $tbName WHERE to_uid=:to_uid AND coin=:coin AND gold =:gold AND item_data=:item_data";

		$db = Hapyfish2_Db_Factory::getDB($uid);
        $rdb = $db['r'];

		return $rdb->fetchOne($sql, array('to_uid' => $pack['to_uid'], 'coin' => $pack['coin'], 'gold' => $pack['gold'], 'item_data' => $pack['item_data']));
	}
