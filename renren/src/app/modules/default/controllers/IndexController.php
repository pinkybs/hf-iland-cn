...
class IndexController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
		...
		if($userIndulge){
			$this->_redirect("Indulge/indulge");
			exit;
		}
        if ($isnew) {
	        ...
	        if ($isInvite) {	        	
	    		...
	    		if ($inviteUid && $isExists && $inviteUid == $isExists['actor']) {
	    			...
	    			if ($ivniteType == 'INVITE') {
						...
						info_log('inviter:'.$puidMS['uid'].'beinvier:'.$uid, 'invitelog');
	    			}
	    		}
	        }
        } else {
        	...
        	if (!$isAppUser) {
        		...
        		$list = $rest->getInvitationSucList();
        		if ($list) {
        			Hapyfish2_Island_Bll_InviteLog::refresh($uid, $list);
        		}*/
        	}
        }
