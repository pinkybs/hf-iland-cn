...
class ApiController extends Zend_Controller_Action
{
     ...
     *
     */
    public function harvestallplantAction()
    {
        $uid = $this->uid;
    ...
    public function adduserindulgeAction()
    {
    	$uid = $this->uid;
		$key = 'i:u:indulge:t' . $uid;
		$lock = Hapyfish2_Cache_Factory::getLock($uid);
    	$ok = $lock->lock($key);
    	$over = 0;
		if (!$ok) {
		...
		$indu = Hapyfish2_Island_Cache_User::setUserindulgeTime($uid);
		if($indu >= 18){
			$over = 1;
			Hapyfish2_Island_Cache_User::addUserIndulge($uid);
		}
		$lock->unlock($key);
		$resultVo = array('status'=>1);
		$result = array('resultVo'=>$resultVo, 'over'=>$over);
		$this->echoResult($result);
    }
