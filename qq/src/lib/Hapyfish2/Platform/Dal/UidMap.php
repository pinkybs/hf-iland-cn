...
/**
 * uid map between with island and platform
 *
 *
 * @package    Dal
...
class Hapyfish2_Platform_Dal_UidMap
{
    ...
    public function getSequence($puid)
    {
    	...
    	$sql = "UPDATE seq_uid SET id=LAST_INSERT_ID(id+16) WHERE `name`=:name";
    ...
    public function newUser($uid, $puid, $time)
    {
        ...
        $sql = "INSERT INTO $tb(uid, puid, create_time) VALUES($uid, UNHEX(:puid), $time)";
    ...
    {
        $tb = $this->getTableName($puid);
    	$sql = "SELECT uid,status FROM $tb WHERE puid=UNHEX(:puid)";
    ...
    {
        $tb = $this->getTableName($puid);
    	$sql = "UPDATE $tb SET status=$status WHERE puid=UNHEX(:puid)";
