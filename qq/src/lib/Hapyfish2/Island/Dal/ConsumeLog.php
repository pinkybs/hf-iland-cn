...
class Hapyfish2_Island_Dal_ConsumeLog
{
    ...
    public function getGoldByCid($uid, $yearmonth, $cid)
    {
    	$tbname = $this->getGoldTableName($uid, $yearmonth);
    	$sql = "SELECT cost,summary,create_time,cid FROM $tbname WHERE uid=:uid AND cid=:cid ORDER BY create_time DESC";
    	
        $db = Hapyfish2_Db_Factory::getDB($uid);
        $rdb = $db['r'];
    	
        return $rdb->fetchAll($sql, array('uid' => $uid, 'cid' => $cid));
    }
}