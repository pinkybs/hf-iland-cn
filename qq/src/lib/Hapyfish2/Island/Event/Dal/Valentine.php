...
class Hapyfish2_Island_Event_Dal_Valentine
{
    ...
    public function get($uid)
    {
        ...
        $rdb = $db['r'];
		$sql = "SELECT uid,rose,rose_tot1,rose_tot2,rose_tot3,gain_pisces FROM island_user_event_valentine_qixi WHERE uid=:uid";
    ...
    public function insert($uid, $info)
    {
        $db = Hapyfish2_Db_Factory::getDB($uid);
        $wdb = $db['w'];
    	return $wdb->insert('island_user_event_valentine_qixi', $info);
    }
    ...
    public function delete($uid)
    {
    	$db = Hapyfish2_Db_Factory::getDB($uid);
        $wdb = $db['w'];
    	$sql = "DELETE FROM island_user_event_valentine_qixi WHERE uid=:uid";
    ...
    public function update($uid, $info)
    {
        ...
        return $wdb->update('island_user_event_valentine_qixi', $info, $where);
    }
    ...
    public function updateByField($uid, $field, $change)
    {
        $db = Hapyfish2_Db_Factory::getDB($uid);
        $wdb = $db['w'];
		$sql = "UPDATE island_user_event_valentine_qixi SET $field = $field + :change WHERE uid=:uid ";
    ...
    public function updateByMultipleField($uid, $param)
    {
        ...
        $s1 = join(',', $change);
        $sql = "UPDATE island_user_event_valentine_qixi SET $s1 WHERE uid=:uid ";
        return $wdb->query($sql, array('uid'=>$uid));
    }
	...
	public function getTableName($uid)
    {
    	...
    	return 'island_user_event_valentine_exchange_qixi_' . $id;
    }
