...
class Hapyfish2_Island_Event_Bll_ConsumeExchange
{
    ...
    public static function Exchange($uid,$step){
		...
		$data = explode('*', $detail['window'.$step]);
		$cid = $data[0];
		$num = $data[1];
		$window = 'window'.$step;
		...
		if ($ok) {
			...
			$feed = Hapyfish2_Island_Bll_Activity::send('XIAO_FEI', $uid);
			$picUrl = STATIC_HOST.'/apps/island/images/event/exchange.jpg';
			$result = array('result' => array('status' => 1, 'feed' => $feed,),'window' => $window, 'picUrl' => $picUrl);
		}
