...
{
	const TXT001 = '获取万圣节卡牌';
	const TXT002 = '对不起，您的卡片不足，不能兑换此物品~';
	const TXT003 = '恭喜你获得：';
	const TXT004 = '对不起，倒计时没结束，不能领取免费卡牌！';
	const TXT005 = '卡片足够，不需要补齐！';
	const TXT006 = '恭喜你通过补齐兑换获得：';
	const TXT007 = '补齐万圣节卡牌';
	...
	public static function halloWeenInit($uid)
	{
		...
		$url = 'http://sobar.soso.com/t/84531678';
...
//		}
		...
		$result = Hapyfish2_Island_Bll_QpointBuy::getToken($uid, 1099, $needGold);
		return array('result' => $result);
			//刷新领取机会
		Hapyfish2_Island_Event_Cache_HallWitches::refrushCardChance($uid, 1);
		//扣除宝石
		try {
			Hapyfish2_Island_HFC_Achievement::updateUserAchievementByField($uid, 'num_19', $needGold);
	 ...
	 */
	 public static function replenishCard($uid, $groupId)
	 {
		...
		$result = Hapyfish2_Island_Bll_QpointBuy::getToken($uid, 1199, array($needGold, $statistics, $lackCard, $card, $groupId));
		return  array('result' => $result); 
		//发东西
