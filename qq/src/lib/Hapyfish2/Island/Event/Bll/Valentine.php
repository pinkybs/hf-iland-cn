...
{
	const CACHE_KEY_EXCHANGE = 'event_valentine_exchange_list_qixi';
	const CACHE_KEY_RANK = 'event_valentine_rank_list_qixi';
	const TXT001 = '获得情人节礼物：';
	const TXT002 = '金币';
	const TXT003 = '成功领取礼品：';
	const TXT004 = '成功兑换礼品：';
	const TXT005 = '恭喜你获得了情人节礼物 ';
	const TXT006 = '对不起，今天的礼物你已经领取过了哦';
	const TXT007 = '对不起，你还没有鹊羽哦';
	const TXT008 = '鹊羽';
	const TXT009 = '七夕礼包';
	const TXT010 = '牛郎';
	const TXT011 = '织女';
	...
	public static function exchangeRose($uid, $changeType=1)
	{
		...
		*/
	    ...
	    if ($ok) {
			...
			$eventFeed->insert($mkey, array($user['nickname'], $gainName, $now, $uid));
	...
	public static function sendRose($uid, $fids)
	{
		...
		foreach ($aryFid as $fid) {
			$isFriend = Hapyfish2_Platform_Bll_Factory::isFriend($uid, $fid);
			if (!$isFriend) {
				$resultVo['status'] = -1;
        ...
        if (!empty($aryAddFids)) {
        	//send to fid
	        $rowUser = Hapyfish2_Platform_Bll_Factory::getUser($uid);
	        foreach ($aryAddFids as $fid) {
				self::incRose($fid, 1, 2);//friend send
				$title = $rowUser['nickname'].'送给你一根鹊羽';
        	...
        	$resultVo['status'] = -1;
        	$resultVo['content'] = "对不起，因为以下原因赠送羽毛失败：\n1）你已经给TA送过羽毛了\n2）选择的人数太多\n3）羽毛已送完\n请检查，或明天再来";
        	return array('result' => $resultVo);
        }
	}
	...
	public static function begRose($uid, $fids)
	{
		...
		//ask for
		$content = '我在参加快乐岛主七夕情人节“鹊桥之恋”活动，帮忙送我一根鹊羽吧！';
		foreach ($aryFid as $fid) {
    ...
    private static function _synRoseRank($uid, $roseNum)
    {
		...
		$rowUser = Hapyfish2_Platform_Bll_Factory::getUser($uid);
		$info = array($rowUser['nickname'], $roseNum, $uid);
