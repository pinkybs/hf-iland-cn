...
class Hapyfish2_Island_Bll_Shop
{
    ...
    public static function buyIslandArray($uid, $islandArray)
    {
        	...
        	if ($itemType == 1) {
	            ...
	            if ($bgInfo && $bgInfo['can_buy'] == 1) {
	                ...
	                if ($bgInfo['price_type'] == 1) {
					  ...
					  /*  if ( $isPlatformVip == 1 && $bgInfo['cheap_price'] > 0 ) {
	            ...
	            if ($buildingInfo && $buildingInfo['can_buy'] == 1) {
	                ...
	                if ($buildingInfo['price_type'] == 1) {
					  ...
					  /*  if ( $isPlatformVip == 1 && $buildingInfo['cheap_price'] > 0 ) {
	            ...
	            if ($plantInfo && $plantInfo['can_buy'] == 1) {
	                ...
	                if ($plantInfo['price_type'] == 1) {
					  ...
					  /*  if ( $isPlatformVip == 1 && $plantInfo['cheap_price'] > 0 ) {
        ...
        if ($needGold > 0) {
	        ...
	        if ( $isPlatformVip != $isVip ) {
				$result['content'] = '您的黄钻信息已有更新，请先刷新游戏再来购买吧。';
				return $result;
	        }
    ...
    public static function buyItemArray($uid, $itemArray)
    {
        ...
        for ($i=0, $iCount = count($itemArray); $i<$iCount; $i++ ) {
            ...
            //building
                ...
                if ($cardInfo['price_type'] == 1) {
                	...
                	/*if ( $isPlatformVip == 1 && $cardInfo['cheap_price'] > 0 ) {
        ...
        if ($needGold > 0) {
            /* edit for Qpoint pay by zx @20111104 -tag-QPOINT
	        $balanceInfo = Hapyfish2_Island_Bll_Gold::get($uid, true);
	        if (!$balanceInfo) {
	        	$result['content'] = 'serverWord_1002';
	        	return $result;
		...
		if ($costGold > 0) {
		    ...
		    if (count($cardArray)>0) {
		        $result = Hapyfish2_Island_Bll_QpointBuy::getToken($uid, 399, $cardArray);
		        return $result;
		    }
    ...
    public static function buyBuildingInWarehouse($uid, $buildingArray, $isVip = 0, $userLevel = 0)
    {
    	...
    	foreach ($buildingArray as $building) {
			...
			if ($building['price_type'] == 1) {
				...
				$ok = Hapyfish2_Island_Bll_Gold::consume($uid, $payInfo);
				if ($ok) {
					$ok2 = Hapyfish2_Island_HFC_Building::addOne($uid, $newBuilding);
					if ($ok2) {
			        	$goldInfo = array(
			        		'uid' => $payInfo['uid'],
			        		'cost' => $payInfo['amount'],
        	...
        	if ($plant['price_type'] == 1) {
				...
				if ($ok) {
					...
					if ($ok2) {
						...
						//买一送一
						$valEndTime = strtotime('2012-02-20 23:59:59');
						if ($now <= $valEndTime) {
							if (in_array($plant['cid'], array(137332, 137432))) {
								$compensation = new Hapyfish2_Island_Bll_Compensation();
								
								if ($plant['cid'] == 137332) {
									$tpsendCid = 137432;
								} else if ($plant['cid'] == 137432) {
									$tpsendCid = 137332;
								}
									
								$compensation->setItem($tpsendCid, 1);
								$valOK = $compensation->sendOne($uid, '购买“' . $plant['name'] . '”赠送：');
								
								if ($valOK) {
									//统计赠送福娃
									info_log($uid . ':' . $tpsendCid, 'valPlant-Send-' . $plant['cid']);
								}
							}
						}
					} else {
    ...
    public static function buyPlantInWarehouse($uid, $plantArray, $isVip = 0, $userLevel = 0)
    {
        ...
        foreach ($plantArray as $plant) {
        	...
        	if ($plant['price_type'] == 1) {
				...
				$ok = Hapyfish2_Island_Bll_Gold::consume($uid, $payInfo);
				if ($ok) {
					$ok2 = Hapyfish2_Island_HFC_Plant::addOne($uid, $newPlant);
					if ($ok2) {
			        	$goldInfo = array(
			        		'uid' => $payInfo['uid'],
			        		'cost' => $payInfo['amount'],
    ...
    public static function buyCard($uid, $cardArray, $isVip = 0, $userLevel = 0)
    {
        ...
        foreach ($cardArray as $card) {
			...
			if ($card['price_type'] == 1) {
		        ...
		        $ok = Hapyfish2_Island_Bll_Gold::consume($uid, $payInfo);
				if ($ok) {
					$ok2 = Hapyfish2_Island_HFC_Card::updateUserCard($uid, $userCards, true);
				    if ($ok2) {
			        	$goldInfo = array(
			        		'uid' => $payInfo['uid'],
			        		'cost' => $payInfo['amount'],
    ...
    /**
     * buy island array for QQ,Q点直购
     *
     * @param integer $uid
     * @param array $islandArray
     * @return array
     */
    public static function buyIslandArrayForQpoint($uid, $islandArray)
    {
        $result = array('status' => -1);
        ...
        if ($needGold > 0) {
            ...
            /* //edit for Qpoint pay by Nick @20111107 -tag-QPOINT  */
            // create qpoint order
            $tokenArray = array('price' => $needGold,
                                'isVip' => $isVip,
                                'userLevel' => $userLevel,
                                'userCurrentIsland' => $userCurrentIsland,
                                'backgroundAry' => $buyBackgroundAry, 
                                'buildingAry' => $buyBuildingAry,
                                'plantAry' => $buyPlantAry);
            $result = Hapyfish2_Island_Bll_QpointBuy::getToken($uid, 1299, $tokenArray);
            return $result;
        }
