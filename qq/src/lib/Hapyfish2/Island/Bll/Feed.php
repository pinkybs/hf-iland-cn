...
class Hapyfish2_Island_Bll_Feed
{
	...
	public static function insertMiniFeed($feed)
	{
	    ...
	    //1 - qzone, 2- pengyou
	    $newfeed[] = (defined('PLATFORM_SOURCE') && '1' == PLATFORM_SOURCE) ? 1 : 2;
    ...
    protected static function buildFeed(&$feeds)
    {
    	...
    	for($i = 0, $count = count($feeds); $i < $count; $i++) {
        	...
        	//if is qzone feed
        	$platform = 2;
        	if (isset($feeds[$i]['platform']) && $feeds[$i]['platform'] == 1) {
        	    $platform = 1;
        	}
        	$title = self::buildTemplate($feeds[$i]['actor'], $feeds[$i]['target'], $tplTitle, $feedTitle, $template_id, $platform);
    	    if ($title) {
                $feeds[$i]['title'] = $title;
            }
            else {
            ...
            unset($feeds[$i]['platform']);
        }

        return $feeds;
    }

    protected static function buildTemplate($actor_id, $target_id, $tplTitle, $feedTitle, $template_id, $platform)
    {
        if ($feedTitle == null) {
            $feedTitle = array();
        }
        ...
        $actor = Hapyfish2_Platform_Bll_Factory::getUser($actor_id);
        ...
        if ($platform != PLATFORM_SOURCE) {
            $preName = (($platform == 1) ? '空间:' : '朋友:');
        }
        $feedTitle['actor'] = $preName . $actor_name;

        if ($target_id) {
            $target = Hapyfish2_Platform_Bll_Factory::getUser($target_id);
