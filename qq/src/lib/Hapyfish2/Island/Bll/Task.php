...
class Hapyfish2_Island_Bll_Task
{
    ...
    public static function readTask($uid)
    {
    	...
    	foreach ($achievementTask as $id => $task) {
    		if(in_array($id, array(3068, 3069, 3070, 3083, 3084, 3085, 3086, 3087,3099))) {
				continue;
			}
				$field = 'num_' . $task['need_field'];
        ...
        if (is_file(LIB_DIR . '/' . $implFile)) {
            ...
            if ($result['status'] == 1) {
                ...
                if ($taskType == 2) {
                    ...
                    $result['feed'] = Hapyfish2_Island_Bll_Activity::send('MISSION_COMPLETE', $uid);
                } else if ($taskType == 3) {
                    unset($result['title']);
					...
					$result['feed'] = Hapyfish2_Island_Bll_Activity::send('USER_OBTAIN_TITLE', $uid, array('title' => $title));
                    $daySend = '';
	                if ( $coinChange > 0 ) {
