...
class Hapyfish2_Island_Bll_Dock
{
    ...
    public static function unlockShip($uid, $shipId, $pid, $priceType)
    {
        ...
        if ($priceType == 1) {
           ...
           $resultVo['expChange'] = $addExp;
           $results['result'] = $resultVo;
           $feed = Hapyfish2_Island_Bll_Activity::send('BOAT_LEVEL_UP', $uid);
           $sendGift = 'unlockShip';
           $result = Hapyfish2_Island_Bll_QpointBuy::getToken($uid, 299, array($pid, $shipId, $shipInfo, $shipList, $addExp));
           $results['result']['status']= isset($result['status'])?$result['status']:-1;
		   $results['result']['content']= isset($result['content'])?$result['content']:-1;
		   $results['result']['urlParams']= isset($result['urlParams'])?$result['urlParams']:-1;
   ...
   public static function expandPosition($uid)
    {
		...
		$friendInfo = Hapyfish2_Platform_Bll_Factory::getFriend($uid);
