...
class Hapyfish2_Island_Bll_DailyAward
{
    ...
    public static function getAwards($uid)
    {
		...
		if(!$isFan) {
			$puidMS = Hapyfish2_Platform_Cache_User::getUser($uid);
			$dalFan = Hapyfish2_Island_Event_Dal_Fans::getDefaultInstance();
    		$isFan = $dalFan->checkFans($puidMS['puid']);
    	...
    	try {
			...
			if ($dailyReward && $dailyReward['items'] && $dailyReward['date'] == $nowDate && empty($dailyReward['gained'])) {
				if(16 != count($dailyReward['items'])){
					...
					$result['items'] = $dailyReward['items']?$dailyReward['items']:self::_getTodayAwardsItem($result['awardNum']);
				}
