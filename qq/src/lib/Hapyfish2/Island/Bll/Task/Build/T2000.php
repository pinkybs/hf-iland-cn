...
class Hapyfish2_Island_Bll_Task_Build_T2000 implements Hapyfish2_Island_Bll_Task_Interface
{
    /**
     * check user task
     *
    ...
    public function check($uid, $taskId)
    {
        ...
        $plantCount = 0;
        if ($taskInfo['need_num'] == 1) {
        	foreach ($userPlantListByItemKind as $item) {
				if ($item['item_id'] == $taskInfo['item_id'] && $item['level'] >= $taskInfo['item_level']) {
					$plantCount = 1;
					break;
				}
			}
        ...
        else if ( $taskInfo['need_num'] > 1 ) {
			foreach ($userPlantListByItemKind as $item) {
				if ($item['item_id'] == $taskInfo['item_id'] && $item['level'] >= $taskInfo['item_level'] ) {
					$plantCount += 1;
				}
			}
            if ($plantCount < $taskInfo['need_num']) {
                $result['content'] = 'serverWord_150';
                return $result;
            }
        }
