...
class Hapyfish2_Island_Bll_SearchFriend
{
	...
	public static function addToQzoneFriendSearch($uid)
	{
		$key = 'i:u:qz:searchFriend';
		$cache = self::getBasicMC();
		$time = time();
		$data = array();
		$m = date('i',$time);
    ...
    public static function getSearchFriend()
    {
		...
		if($friendList){
			foreach($friendList as $k1=>$v1){
				$user = Hapyfish2_Platform_Bll_Factory::getUser($v1);
				$userVO = Hapyfish2_Island_HFC_User::getUserVO($v1);
				$list[$k1]['uid'] = $v1;
				$list[$k1]['name'] = $user['nickname'];
				$list[$k1]['face'] = $user['figureurl'];
				$list[$k1]['islandLevel'] = $userVO['level'];
				$list[$k1]['link'] = 'http://appmng.xiaoyou.qq.com/cgi-bin/xyapp/xy_third_jump_HomePage.cgi?openid=' . $user['openid'];
			}
		}
		return $list;
    }
    
 public static function getQzoneSearchFriend()
    {
    	$data = array();
    	$key = 'i:u:qz:searchFriend';
		$cache = self::getBasicMC();
