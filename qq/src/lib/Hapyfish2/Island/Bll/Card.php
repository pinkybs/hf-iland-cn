...
class Hapyfish2_Island_Bll_Card
{
	...
	public static function useCard($uid, $ownerUid, $cid, $itemId)
	{
		...
		if ($id == 271) {
			...
			$resultVo = self::balanceOneCard($uid, $cid, $itemId);
		} else if ($id == 932) {
			$resultVo = self::balanceAllCard($uid, $cid);
	...
	public static function damageCard($uid, $ownerUid, $cid, $itemId)
	{
		...
		try {
            ...
            $resultVo['levelUp'] = $levelUp['levelUp'];
            $resultVo['islandLevelUp'] = $levelUp['islandLevelUp'];
            $owner = Hapyfish2_Platform_Bll_Factory::getUser($ownerUid);
            $resultVo['feed'] = Hapyfish2_Island_Bll_Activity::send('DAMAGE_PLANT', $uid, array('owner' => $owner['name']));
		} catch (Exception $e) {
		}
	...
	public static function upgradeCard($uid, $cid, $itemId)
	{
        ...
        //单身节活动,屏蔽对婚纱店使用建设卡
//        if (($userPlant['cid'] == 121032) || ($userPlant['cid'] == 121132)) {
//        	$resultVo['content'] = '该建筑不能使用建设卡升级！';
//        	return array('resultVo' => $resultVo);
//        }
		...
		try {
            ...
            $resultVo['itemBoxChange'] = true;
            $resultVo['feed'] = Hapyfish2_Island_Bll_Activity::send('BUILDING_LEVEL_UP', $uid);
		} catch (Exception $e) {
			$resultVo['content'] = 'serverWord_110';
            return array('resultVo' => $resultVo);
		}
	...
	public static function balanceOneCard($uid, $cid, $itemId)
	{
		$resultVo = array('status' => -1);
        $itemType = substr($itemId, -2, 1);
        $itemId = substr($itemId, 0, -2);
        ...
        $cardInfo = Hapyfish2_Island_Cache_BasicInfo::getCardInfo($cid);
		if (!$cardInfo) {
			 $resultVo['content'] = 'serverWord_2001';
			return array('resultVo' => $resultVo);
		}
        ...
        $plantInfo = Hapyfish2_Island_Cache_BasicInfo::getPlantInfo($userPlant['cid']);
        if (!$plantInfo) {
        	$resultVo['content'] = 'serverWord_2002';
        	return  array('resultVo' => $resultVo);
        }
        $now = time();
		...
		$usecard = Hapyfish2_Island_HFC_Card::useUserCard($uid, $cid, 1, $userCard);
		if (!$usecard) {
			$resultVo['content'] = 'serverWord_110';
			return array('resultVo' => $resultVo);
		}
		...
		$clientPlantInfo = Hapyfish2_Island_Bll_Plant::handlerPlant($userPlant, $now);
		return array('resultVo' => $result, 'buildingVo' => $clientPlantInfo);
	}


	public static function balanceAllCard($uid, $cid)
	{
		$result = array('status' => -1);

        $cardInfo = Hapyfish2_Island_Cache_BasicInfo::getCardInfo($cid);
		if (!$cardInfo) {
			$result = array('content' => 'serverWord_2001');
		...
		if(!$data['plants']){
			$result['content'] = 'serverWord_1005';
			return array('resultVo' => $result);
		}
		$coinChange = 0;
		$expChange = 0;
