...
class Hapyfish2_Island_Tool_SaveOldUserCache
{
    ...
    public static function savedbAllUser($dbId)
    {
        ...
        //2012-04-03 00:00:05
        $startTime = 1333382405;
        
        $db = Hapyfish2_Island_Dal_Savediy::getDefaultInstance();
        
        //for($i=0;$i<4;$i++) {
        info_log('# db - #:'.$dbId, 'savedbAllUser_'.$dbId);
            for($j=0;$j<10;$j++) {
            	$count = 0;
                ...
                for ( $k=0;$k<$kCount;$k++) {
                	self::saveOne($uidList[$k]['uid']);
                	$count++;
                    info_log('uid:'.$uidList[$k]['uid'].'--count:'.$count, 'savedbAllUser_'.$dbId);
                }
            }
        //}
    ...
    public static function savedbAllUserTest($uid)
    {
        info_log('# savedbAllUser - start - #', 'savedbAllUser_test');
        //2011-11-19 00:00:00
        $startTime = 1333382405;
        ...
        for($i=0;$i<4;$i++) {
        info_log('# db - #', 'savedbAllUser_'.$dbId);
            for($j=0;$j<10;$j++) {
                $dbId = $i;
                info_log('savedbAllUser:tableId-'.$j, 'savedbAllUser_'.$dbId);
                $uidList = $db->getUidListByPage($dbId, $j, $startTime);
                ...
                for ( $k=0;$k<$kCount;$k++) {
                    self::saveOne($uidList[$k]['uid']);
                    info_log('uid:'.$uidList[$k]['uid'], 'savedbAllUser_'.$dbId);
                }
            }
        }*/
        var_dump($uid);
        ...
        var_dump('end');
        var_dump('<br/>');
        info_log('uid:'.$uid, 'savedbAllUser_test');
