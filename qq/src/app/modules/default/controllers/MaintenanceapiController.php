...
{	
	...
	public function loadshipdataAction()
	{
		Hapyfish2_Island_Cache_BasicInfo::loadShipList();

		$data = array('result' => 'OK');
		$this->echoResult($data);
	}
	
	public function loadlocalshipdataAction()
	{
		$cache = Hapyfish2_Island_Cache_BasicInfo::getBasicMC();
		$localcache = Hapyfish2_Cache_LocalCache::getInstance();
	...
	public function dumpusersAction()
	{
		$uid = $this->_request->getParam('uid');
		if (empty($uid)) {
			echo 'uid empty!';
	...
	{
		$btl_id = $this->_request->getParam('btl_id','0,1,2');
	...
	public function loadalltestAction()
	{
		$this->isTest = 1;
		self::loadislandlevelAction();
		self::loadstepgiftlevelAction();
		self::loadgiftlevelAction();
		self::loadcardlistAction();
		self::loadfeedtemplateAction();
		self::loadlotteryitemAction();
		self::loadshipAction();
		self::loadachievementtaskAction();
		self::loadtaskdataAction();
		self::loadtitlelistAction();
		echo 'ok';
		exit;
	}
	
	public function loadlocalalltestAction()
	{
		$this->isTest = 1;
		self::loadlocalislandlevelAction();
		self::loadlocalstepgiftlevelAction();
		self::loadlocalgiftlevelAction();
		self::loadlocalcardlistAction();
		self::loadlocalfeedtemplateAction();
		self::loadlocallotteryitemAction();
		self::loadlocalshipAction();
		self::loadlocalachievementtaskAction();
		self::loadlocaltaskdataAction();
		self::loadlocaltitlelistAction();
		echo 'ok';
		exit;
	}
